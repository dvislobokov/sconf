...
// реализует отдельный пакет (sconf/vault), который остаётся опциональным —
// ядро sconf от него не зависит.
//
...
//
// Резолвер (sconf/vault, подключается blank-импортом) после бинда обходит
// структуру, находит поля-Resolvable и заполняет их из Vault.
package secret
...
// Resolvable реализуется типами полей секретов. Резолвер (sconf/vault) для
